...
function HistoryTab() {
  ...
  useEffect(() => { setReviews(getVideoReviews(activeIP?.id)); }, [activeIP]);
        ...
        <Card key={r.id}>
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button onClick={() => { deleteVideoReview(r.id); setReviews(getVideoReviews(activeIP?.id)); }} className="text-[#999] hover:text-[#A32D2D]"><Icon name="trash" size="sm" /></button>
            </div>
          </div>
