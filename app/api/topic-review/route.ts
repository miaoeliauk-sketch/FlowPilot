...
子维度（每项0-10分，统一规则：分数越高代表对该IP越有利）：执行容易度（越容易执行分越高）、素材易得度（素材越好获取分越高）、差异化程度（同质化越低分越高）、创新空间（越大分越高）。
最终分数 = (各维度之和 / 维度数量) × 10，结果取整。
严格按JSON格式输出。`,
    userPromptTemplate: (topic: string, ipBlock: string) => `${ipBlock}
...
严格按以下JSON格式输出：
{
  ...
  "dims": [
    {"label": "执行容易度", "score": 数字},
    {"label": "素材易得度", "score": 数字},
    {"label": "差异化程度", "score": 数字},
    {"label": "创新空间", "score": 数字}
  ],
  "vote": "支持或保留意见或反对"
}`,
...
子维度（每项0-10分，统一规则：分数越高代表对该IP越有利）：赛道宽松度（竞争越不激烈分越高）、差异化空间（越高分越高）、出圈机会（越高分越高）。
最终分数 = (各维度之和 / 维度数量) × 10，结果取整。
严格按JSON格式输出。`,
    userPromptTemplate: (topic: string, ipBlock: string) => `${ipBlock}
...
子维度（每项0-10分）：利益点、冲突感、反差设计、戏剧感（内容展开是否有剧情感、能否让人想看到最后）、情绪触发、结果导向。
冲突感、反差设计和戏剧感不许硬编；选题本身没有，就如实给低分，并在结论中说明可以如何改善。
最终分数 = (各维度之和 / 维度数量) × 10，结果取整。
严格按JSON格式输出。`,
    userPromptTemplate: (topic: string, ipBlock: string) => `${ipBlock}
...
严格按以下JSON格式输出：
{
  ...
  "failProbability": 数字（0-100的整数，代表失败概率百分比。必须与riskLevel一致：低风险<30，中风险30-60，高风险>60）,
