...
import { ReviewMetrics, ReviewGrade, ReviewPerformanceType, ConfidenceTier } from "@/lib/types";
...
interface HistoricalAvg { views: number | null; likes: number | null; comments: number | null; favorites: number | null; count: number; }
...
interface RequestBody {
  ...
  ipContext?: { name: string; positioning: string; contentDirection: string[] } | null;
...
const PROMPT = (body: RequestBody, hasScipt: boolean) => `
视频信息：
标题：${body.title || "（未提供）"}
...
${body.knowledgeContext && body.knowledgeContext.length > 0
  ? `知识库可参考案例（可在Layer2中引用）：${body.knowledgeContext.map(k => `[${k.category}]${k.title}`).join("、")}`
...
请严格按以下JSON格式输出六层分析：
{
  ...
  "layer3": {
    "hasScriptText": ${hasScipt},
    "noScriptReason": "${hasScipt ? "" : "用户未提供口播稿或逐字稿，内容结构分析需要完整文本作为依据，无法基于标题和数据做结构评分"}",
    "titleAnalysis": {"score": 0-10, "feedback": "标题分析", "suggestion": "改进建议"},
    "hookAnalysis": {"score": ${hasScipt ? "0-10" : 0}, "feedback": "${hasScipt ? "开头钩子分析（基于逐字稿前几句）" : "无文本，无法评分"}", "suggestion": ""},
    "middleAnalysis": {"score": ${hasScipt ? "0-10" : 0}, "feedback": "${hasScipt ? "中段价值分析" : "无文本，无法评分"}", "suggestion": ""},
    "endingAnalysis": {"score": ${hasScipt ? "0-10" : 0}, "feedback": "${hasScipt ? "结尾分析" : "无文本，无法评分"}", "suggestion": ""}
  },
  "layer4": {
...
export async function POST(req: NextRequest) {
  ...
  const hasMetrics = !!body.metrics && Object.values(body.metrics).some(v => (v as number) > 0);
  if (!body.title?.trim() && !hasMetrics) {
    return NextResponse.json({ error: "请至少提供视频标题或一项真实数据" }, { status: 400 });
  }

  const hasScript = !!(body.scriptText?.trim());
  const calledAt = new Date().toISOString();
  const apiMeta = { apiCalled: true, calledAt, model: MODEL, ipUsed: body.ipContext?.name ?? null, mockHit: false };

  try {
    const raw = await callDeepSeek(SYSTEM, PROMPT(body, hasScript), 4000, 0.3, apiKey);
    const parsed = parseJSON(raw, null as Record<string, unknown> | null);
    if (!parsed) throw new Error("AI返回内容解析失败，请重试");

    return NextResponse.json({ ...parsed, hasScript, apiMeta });
  } catch (err) {
