...
4. PARTIAL：有明确核心判断，但本次切入仍缺推理过程或关键概念解释。PARTIAL必须引用至少一条kind为claim的原始内容，不能只引用reasoning、case、expression或topic。
5. NONE：没有明确核心判断。只有相似话题、案例、数据、表达习惯或IP人设也必须判NONE，不能因为“方向接近”判为PARTIAL。
...
export function isCoverageSource(value: unknown): value is CoverageSourceReference {
...
export async function analyzeScriptCoverage(input: {
  topic: string;
  angle: string;
  ...
  if (input.sources.length === 0) {
    return createEmptyCoverageAssessment(input.topic);
  }

  const modelResult = await callStructuredDeepSeek({
    systemPrompt: SYSTEM_PROMPT,
    userPrompt: buildPrompt(input.topic, input.angle, input.sources),
    parse: response => parseCoverageAssessment(response, input.sources),
    apiKey: input.apiKey,
    maxTokens: 1800,
    temperature: 0,
    timeoutMs: 60_000,
